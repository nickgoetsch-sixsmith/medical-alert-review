...
// Header bar shows only the top links so the nav never wraps; the
// remaining pages stay reachable from the menu, footer, and homepage grid.
    ...
    <header className="bg-[#1a5f7a] text-white">
      <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex gap-1 text-sm font-medium">
          {primaryLinks.map((l) => (
            <a key={l.href} href={l.href} className="hover:underline whitespace-nowrap px-2 py-2.5">
              {l.label}
            </a>
          ))}
        </nav>
      ...
      {/* Mobile dropdown */}
      {open && (
        <div className="lg:hidden bg-[#134a61] border-t border-white/20">
